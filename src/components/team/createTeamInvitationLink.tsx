...
> = ({ teamId, tokenId, updateTeam }) => {
  ...
  const createTeamInvitationLink = () => {
    ...
    setInvitationLink(`https://${baseInvitationLink}?tokenId=${tokenId}`);
    updateTeam({ tokenId });
  };
  ...
  return (
    <div className="rounded-xl bg-slate-100 dark:bg-slate-800 dark:text-white p-4 sm:p-8">
      ...
      <button
        className="btn mt-2 px-3 py-2 text-sm sm:text-base"
        onClick={createTeamInvitationLink}
      >
        Generate
      </button>
      {invitationLink && (
        <div className="mt-2 px-3 py-2 text-sm sm:text-base">
          {invitationLink}
        </div>
      )}
